...
use open_timeline_games::which_date::{WhichDateGame, WhichDateGameVariant, YearOrDecade};
...
impl WhichDateGameGui {
    ...
    fn draw_question(&mut self, _ctx: &Context, ui: &mut Ui, enabled: bool) {
        let current_question = self.game.current_question();
        ...
        ui.horizontal(|ui| {
            ui.add_enabled_ui(self.state == GameState::NotStarted, |ui| {
                ui.radio_value(
                    ...
                    WhichDateGameVariant::StartDate,
                    "Enter start date",
                );
                ui.radio_value(
                    self.game.variant_mut(),
                    WhichDateGameVariant::EndDate,
                    "Enter end date",
                );
                ui.separator();
                ui.radio_value(
                    self.game.year_or_decade_mut(),
                    YearOrDecade::Decade,
                    "Enter decade",
                );
                ui.radio_value(
